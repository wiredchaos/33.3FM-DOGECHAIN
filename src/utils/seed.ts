...
export const SEED_VERSION = '2026-06-05-b';

export const CITIZENS: Artist[] = [
  {
    id: 'red-fang',
    name: 'DJ RED FANG',
    handle: '@redfang',
...
export function buildCompletedAgents(r: typeof RAW_RELEASES[0], citizen: Artist) {
  const now = Date.now();
  const fingerprintNum = Math.random().toString(16).slice(2, 18);
  const upcNum = Math.floor(Math.random() * 1e11).toString().padStart(12, '0');
  const isrcNum = Math.floor(Math.random() * 9000000) + 1000000;
  return {
    release: {
      status: 'complete' as const,
      completedAt: now,
      output: {
        isrc: `WC-33FM-${isrcNum}`,
        upc: `8${upcNum}`,
        fingerprint: `sha256:${fingerprintNum}`,
...
export function ensureSeed() {
  ...
  const currentVersion = localStorage.getItem('wc_seed_version_ts');
  if (currentVersion === SEED_VERSION) return;

  // 1. Seed Artists
  let existingArtists: Artist[] = [];
  try {
    existingArtists = JSON.parse(localStorage.getItem('wc_artists') || '[]');
  } catch (e) {}
  const artistMap = new Map(existingArtists.map(a => [a.id, a]));
  CITIZENS.forEach(c => {
    if (!artistMap.has(c.id)) {
      artistMap.set(c.id, c);
    }
  });
  localStorage.setItem('wc_artists', JSON.stringify(Array.from(artistMap.values())));

  // 2. Seed OacReleases
  let existingReleases: OacRelease[] = [];
  try {
    existingReleases = JSON.parse(localStorage.getItem('wc_oac_releases') || '[]');
  } catch (e) {}
  const releaseMap = new Map(existingReleases.map(r => [`${r.artist}::${r.trackTitle}`, r]));

  const now = Date.now();
  const dayMs = 86400000;
  RAW_RELEASES.forEach((r, idx) => {
    const citizen = CITIZENS.find(c => c.id === r.artist_id)!;
    const key = `${citizen.name}::${r.title}`;
    if (!releaseMap.has(key)) {
      const daysAgo = (RAW_RELEASES.length - idx) * 1.5 + Math.random() * 0.5;
      const releaseId = `rel_seed_${r.artist_id}_${idx}`;
      releaseMap.set(key, {
        id: releaseId,
        trackTitle: r.title,
        artist: citizen.name,
  ...
  localStorage.setItem('wc_oac_releases', JSON.stringify(Array.from(releaseMap.values()).slice(0, 100)));

  // 3. Seed Radio Tracks
  let existingTracks: RadioTrack[] = [];
  try {
    existingTracks = JSON.parse(localStorage.getItem('wc_radio_tracks') || '[]');
  } catch (e) {}
  const trackMap = new Map(existingTracks.map(t => [`${t.title}|${t.artist}`, t]));
  RAW_RELEASES.forEach((r, idx) => {
    const citizen = CITIZENS.find(c => c.id === r.artist_id)!;
    ...
    if (!trackMap.has(key)) {
      trackMap.set(key, {
        ...
        chain: 'multi',
        status: 'rotation',
        ingested: now - idx * 1000,
        oacRelease: true,
        mintedAt: now - Math.floor(Math.random() * 30) * dayMs
      });
    }
  });
  localStorage.setItem('wc_radio_tracks', JSON.stringify(Array.from(trackMap.values()).slice(0, 80)));

  // 4. Seed Schedule
  let existingSchedule: Show[] = [];
  try {
    existingSchedule = JSON.parse(localStorage.getItem('wc_radio_schedule') || '[]');
  } catch (e) {}
  if (!existingSchedule.length) {
    const defaultSchedule: Show[] = [
      { time: '22:00', title: 'VOID FREQUENCY', host: 'DJ RED FANG', type: 'LIVE', status: 'live', duration: 60, description: 'Late-night chaos protocol. Industrial void selections from District 7.', created: now },
      { time: '23:00', title: 'PACK SIGNAL', host: 'PACK', type: 'LIVE', status: 'queued', duration: 60, description: 'Sovereignty focus hour, direct lyrics, phone hotlines.', created: now },
      { time: '00:00', title: 'LENS UPLINK', host: 'LENS', type: 'PRE-RECORDED', status: 'queued', duration: 60, description: 'Electronic build focus, zero static noise.', created: now },
      { time: '01:00', title: 'MINT DROP HOUR', host: 'MINT', type: 'LIVE', status: 'queued', duration: 60, description: 'Mellow washes, lo-fi beats, treasury token updates.', created: now },
      { time: '02:00', title: 'CIPHER HOUR', host: 'DJ RED FANG', type: 'VAULT33 GATED', status: 'queued', duration: 60, description: 'V33 secret codes, unreleased master tracks, raw Afrofuturism.', created: now },
      { time: '03:00', title: 'MADISON TAKEOVER', host: 'HUMAN GUEST', type: 'LIVE', status: 'queued', duration: 60, description: 'District 1 takeover. Human citizens on-mic.', created: now }
    ];
    localStorage.setItem('wc_radio_schedule', JSON.stringify(defaultSchedule));
  }

  // 5. Seed Balance
  const existingBalance = localStorage.getItem('wc_xents_balance');
  if (existingBalance === null || Number(existingBalance) === 0) {
    localStorage.setItem('wc_xents_balance', '2500'); // Initial free credits for testing
    localStorage.setItem('wc_xents_ledger', JSON.stringify([
      { ts: now, type: 'bonus', label: 'WELCOMING BONUS CREDITS', xents: 2500 }
    ]));
  }

  // Mark seeded
  localStorage.setItem('wc_seed_version_ts', SEED_VERSION);
}
